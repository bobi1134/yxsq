...
<html>
	...
	<body>
		<div class="dqwz">
			当前位置：帖子管理&gt;&gt;首页精帖管理&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/admin/tz/admin_selectAllIndexHuoTz.action" target="content">去热帖管理</a>
			<div style="width:450px;height:12px;float:right;margin-right:10px">
				<marquee scrollAmount="2"><font color="red">${session_user.nickname}</font>，你好！首页目前只能设置最多10条社区精帖！您可以在所有帖子中选取好的帖子设置为精帖，系统会根据您所选择的所有精帖中按照评论的多少抽取前面10条帖子进行展示！如何更换其他评论较少的帖子成为精帖，您可以删除之前的精帖然后重新选择！</marquee>
			</div>
		</div>
		<div class="ss">
			<form action="${pageContext.request.contextPath}/admin/tz/admin_selectAllIndexJingTz.action" method="post">
		...
		<table width="100%" border="1" style="border:#c2c6cc 1px solid; border-collapse:collapse;">
			...
			<s:iterator value="tzs">
				<tr onMouseOver="move(this);" onMouseOut="out(this);" height="30">
					...
					<td align="center"><a href="${pageContext.request.contextPath}/admin/tz/deleteTojingTz_2.action?id=${id}" title="取消加精"><img src="${pageContext.request.contextPath}/admin_file/images/jing.gif" width="15" height="15" id="jing"/></a></td>
				</tr>
			</s:iterator>
		</table>
		<div class="page">
			<xy_2:pager_2 pageIndex="${pageModel.pageIndex}" pageSize="${pageModel.pageSize}" recordCount="${pageModel.recordCount}" submitUrl="${pageContext.request.contextPath}/admin/tz/admin_selectAllIndexJingTz.action?pageModel.pageIndex={0}&sqName=${sqName}&tzTitle=${tzTitle}"/>
		</div>
	</body>
</html>